(ns enveng.actor
  "EnvengActor — the ISCO-08 2143 environmental engineer actor as
  a `langgraph.graph/state-graph` (per ADR-2607011000 / CLAUDE.md
  Actors section). One graph run = one environmental engineering
  ...
  The unconditional invariant: the EnvengAdvisor can never directly
  issue a certified engineering design, certify compliance, or dispatch
  a robot action the EnvengGovernor refuses — every commit-record!
  call is gated behind `:decide`."
...
(defn build-graph
  "Build a compiled EnvengActor graph. `store` implements
  `enveng.store/Store`. `advisor` implements
  `enveng.advisor/Advisor` (defaults to `mock-advisor`).
...
(defn run-request!
  "Run one environmental engineering operation request to completion or
